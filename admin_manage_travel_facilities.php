...
$destinations = $conn->query("
    SELECT dest_id, title FROM destinations ORDER BY title
...
if ($destFilter) { $where .= " AND f.dest_id=?"; $params[]=$destFilter; }
if ($typeFilter) { $where .= " AND f.transport_type=?"; $params[]=$typeFilter; }

/* ---------- Fetch ---------- */
$stmt = $conn->prepare("
    SELECT f.*, d.title AS destination
    FROM travel_facilities f
    JOIN destinations d ON f.dest_id=d.dest_id
    $where ORDER BY f.facility_id DESC
");
$stmt->execute($params);
...
.filters button{background:#2563eb;color:#fff;border:none;font-weight:600}

/* ===== DESKTOP TABLE (UNCHANGED) ===== */
.table-wrap{overflow-x:auto}
table{
    width:100%;min-width:750px;background:#fff;
...
.facility-card h3{margin:0 0 8px}
.facility-card p{margin:6px 0}
...
.modal-actions{margin-top:18px;display:flex;gap:12px;justify-content:center}
.m-btn{padding:10px 18px;border-radius:999px;border:none;font-weight:700}
...
@media(max-width:900px){
    ...
    .overlay.show{display:block}
    .content{margin-left:0;padding:24px;padding-top:80px}
...
<div class="topbar">
    ...
    <span class="topbar-title">Travel Facilities</span>
</div>
...
<?= $msg==='deleted'?'Deleted 🗑️':($msg==='updated'?'Updated ✅':'In use ❌') ?>
</div>
<script>
...
<option <?= $typeFilter==$t?'selected':'' ?>><?= $t ?></option>
<?php endforeach; ?>
</select>
...
<thead>
<tr>
<th>Destination</th><th>Transport</th><th>Provider</th>
...
<p><b>Transport:</b> <?= htmlspecialchars($f['transport_type']) ?></p>
<p><b>Provider:</b> <?= htmlspecialchars($f['provider_name'] ?: '—') ?></p>
<p><b>Duration:</b> <?= htmlspecialchars($f['duration'] ?: '—') ?></p>
<p><b>Price:</b> $<?= number_format($f['price'],2) ?></p>
<div class="actions">
...
function openDelete(id){
    deleteLink.href='delete_travel_facility.php?id='+id;
